<template>
  <div id="relearn">
    ...
    <div class="actions" >
      <div class="row">
        <el-autocomplete v-if="isParent(userinfo.getAuth())" clearable
          v-model="queryRelearnCmd.student_id" class="ipt" placeholder="请输入学号"
...
const queryUserBasicinfoHandler = async (query, cb) => {
  ...
  const resp = await queryUserBasicinfoApi({
    info: queryRelearnCmd.value.student_id,
    type: 3
  })
  // 判断返回的数据是否存在
  if (resp && resp.data) {
...
const handleSelectStudent = (item) => {
  queryRelearnCmd.value.student_id = item.label.split('|').map(s => s.trim())[1]
  console.log('Selected:', queryRelearnCmd.value.student_id)
}
</script>
